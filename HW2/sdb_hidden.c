...
void handle_load(char* path_arg) {
    ...
    if (g_child_pid == 0) {
        ...
        if (WIFSTOPPED(g_child_status)) {
            ...
            // ... (處理 target_vma_entry == 0 的情況) ...
            g_entry_point_vma = target_vma_entry; // 例如 0x401650 for anon

            // 計算 g_program_base_address
            if (g_program_is_pie) {
                if (elf_file_entry_rva != 0) { // 例如 anon 的 e_entry 是 0x1650 (RVA)
                    g_program_base_address = g_entry_point_vma - elf_file_entry_rva; // 0x401650 - 0x1650 = 0x400000
                } else {
                    // ... (從 maps 獲取的備用邏輯) ...
                }
            } else { // 非 PIE
                ...
                if (strcmp(g_program_path, "./anon") == 0) {
                    unsigned long long anon_base_from_maps = 0;
                    if (prog_basename) { // Ensure prog_basename is not NULL
                        anon_base_from_maps = get_program_base_address_from_maps(g_child_pid, prog_basename);
                    }

                    if (anon_base_from_maps != 0) {
                        g_program_base_address = anon_base_from_maps;
                    } else if (g_program_is_pie && elf_file_entry_rva != 0) {
                        g_program_base_address = g_entry_point_vma - elf_file_entry_rva;
                    } else { // Fallback for anon if other methods fail to yield 0x400000
                        g_program_base_address = 0x400000;
                    }
                } else if (g_program_base_address == 0 && strcmp(g_program_path, "./hello") == 0) {
                    g_program_base_address = 0x400000;
                }
        }
...
void handle_si() {
    ...
    // 階段 1: 如果上一個命令 (cont/syscall/si) 停在一個已恢復的斷點上，先執行它。
    if (g_is_stepping_over_breakpoint) {
        Breakpoint* bp_that_was_stepped_over = find_breakpoint_at(g_stepped_over_breakpoint_addr);
        ...
        // 2. 重新啟用剛越過的斷點
        if (bp_that_was_stepped_over) {
            enable_breakpoint(bp_that_was_stepped_over);
        }
        g_is_stepping_over_breakpoint = 0; // 清除標誌，因為越過操作已完成
        g_stepped_over_breakpoint_addr = 0;

        if (WIFEXITED(g_child_status) || WIFSIGNALED(g_child_status)) {
            printf("** the target program terminated.\n");
            g_child_pid = 0; /* ... reset other globals ... */
            // 如果在越過時終止，並且越過的是一個斷點，可以考慮在退出前重新啟用它，
            // 但程式已終止，所以可能意義不大。
            return;
        }
        ...
        // 程式未終止，獲取單步執行後的 RIP
        ptrace(PTRACE_GETREGS, g_child_pid, NULL, &g_regs);
        unsigned long long rip_after_stepping_over = g_regs.rip;

        // 3. 檢查這個越過操作之後，是否立即停在了一個新的斷點上
        //    這個 SIGTRAP 是來自於上面 PTRACE_SINGLESTEP 的完成。
        if (WIFSTOPPED(g_child_status) && WSTOPSIG(g_child_status) == SIGTRAP) {
            Breakpoint* bp_hit_after_step_over = NULL;
            Breakpoint* temp_bp_minus_1 = find_breakpoint_at(rip_after_stepping_over - 1);
            if (temp_bp_minus_1 && temp_bp_minus_1->is_enabled) {
                bp_hit_after_step_over = temp_bp_minus_1;
            } else {
                Breakpoint* temp_bp_at_rip = find_breakpoint_at(rip_after_stepping_over);
                if (temp_bp_at_rip && temp_bp_at_rip->is_enabled) {
                    bp_hit_after_step_over = temp_bp_at_rip;
                }
            }

            if (bp_hit_after_step_over) { // 越過一個斷點後，立即又命中了另一個
                printf("** hit a breakpoint at 0x%llx.\n", bp_hit_after_step_over->address);
                if (bp_hit_after_step_over->address != rip_after_stepping_over) { // 校正 RIP
                    g_regs.rip = bp_hit_after_step_over->address;
                    ptrace(PTRACE_SETREGS, g_child_pid, NULL, &g_regs);
                }
                disable_breakpoint(bp_hit_after_step_over); // 恢復新命中的斷點

                // 為這個新命中的斷點設置 stepping_over 狀態，給下一個命令
                g_is_stepping_over_breakpoint = 1;
                g_stepped_over_breakpoint_addr = bp_hit_after_step_over->address;

                disassemble_and_print(g_regs.rip, 5); // 從新命中断点處反組譯
            } else { // 越過斷點後，正常停止，未命中新斷點
                disassemble_and_print(rip_after_stepping_over, 5);
            }
        } else { // 越過斷點後，因其他信號停止
             printf("** child stopped by signal %d after stepping over breakpoint\n", WSTOPSIG(g_child_status));
             disassemble_and_print(rip_after_stepping_over, 5);
        }
        return; // SI 命令完成 (它執行了待處理的越過操作，並處理了之後的狀態)
    }

    // 階段 2: 正常 SI (g_is_stepping_over_breakpoint 為 false，沒有待處理的斷點越過)
    if (ptrace(PTRACE_SINGLESTEP, g_child_pid, NULL, NULL) < 0) {
    ...
    if (WIFSTOPPED(g_child_status)) {
        ptrace(PTRACE_GETREGS, g_child_pid, NULL, &g_regs);
        unsigned long long current_rip_normal_si = g_regs.rip;

        if (WSTOPSIG(g_child_status) == SIGTRAP) {
            Breakpoint* bp_hit_normal_si = NULL;
            Breakpoint* temp_bp_minus_1 = find_breakpoint_at(current_rip_normal_si - 1);
            if (temp_bp_minus_1 && temp_bp_minus_1->is_enabled) {
                bp_hit_normal_si = temp_bp_minus_1;
            } else {
                Breakpoint* temp_bp_at_rip = find_breakpoint_at(current_rip_normal_si);
                if (temp_bp_at_rip && temp_bp_at_rip->is_enabled) {
                    bp_hit_normal_si = temp_bp_at_rip;
                }
            }

            if (bp_hit_normal_si) { // SI 命令自己命中了一個斷點
                printf("** hit a breakpoint at 0x%llx.\n", bp_hit_normal_si->address);
                if (bp_hit_normal_si->address != current_rip_normal_si) { // 校正 RIP (如果停在 bp+1)
                    g_regs.rip = bp_hit_normal_si->address;
                    ptrace(PTRACE_SETREGS, g_child_pid, NULL, &g_regs);
                }
                disable_breakpoint(bp_hit_normal_si); // 恢復原始指令

                // 設置 flag，讓下一個命令處理越過操作
                g_is_stepping_over_breakpoint = 1;
                g_stepped_over_breakpoint_addr = bp_hit_normal_si->address;

                disassemble_and_print(g_regs.rip, 5); // 從斷點處反組譯 (顯示原始指令)
            } else { // 普通單步完成，未命中斷點
                disassemble_and_print(current_rip_normal_si, 5);
            }
        } else {
...
void handle_breakrva(char* offset_str) {
    if (g_child_pid == 0) { printf("** please load a program first.\n"); return; }
    if (!offset_str) { printf("** no offset specified for breakrva.\n"); return; }

    unsigned long long offset = strtoull(offset_str, NULL, 16);


    if (g_program_base_address == 0) {
        if (strcmp(g_program_path, "./anon") == 0) { // 假設 anon 的行為類似 hello
             char* last_slash = strrchr(g_program_path, '/');
             const char* prog_basename = last_slash ? last_slash + 1 : g_program_path;
             unsigned long long temp_base = get_program_base_address_from_maps(g_child_pid, prog_basename);
             if (temp_base != 0) {
                 g_program_base_address = temp_base;
             } else {
    ...
    if (g_program_base_address == 0) {
        if (strcmp(g_program_path, "./anon") == 0) { // 為 anon 提供一個預設的 image base
            g_program_base_address = 0x400000; // 假設 anon 類似 hello
        } else {
